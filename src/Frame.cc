...
uint32_t Frame::nNextId=0;
uint32_t FrameDirect::nNextId=0;
...
FrameDirect::FrameDirect()
{


}

FrameDirect::FrameDirect(const FrameDirect &frame) :
...
FrameDirect::FrameDirect(const cv::Mat &imGray, const double &timeStamp, cv::Mat &K,cv::Mat &distCoef) : mTimeStamp(timeStamp)
...
void FrameDirect::SetPose(cv::Mat Tcw)
{
    mTcw = Tcw.clone();
    UpdatePoseMatrices();
}

void FrameDirect::UpdatePoseMatrices()
{
...
cv::Mat FrameDirect::GetPoseInverse() const
{
    return mTwc.clone();
}
